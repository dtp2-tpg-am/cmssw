...
DTPattern::DTPattern(RefDTPatternHit seedUp, RefDTPatternHit seedDown) : seedUp_(seedUp), seedDown_(seedDown) {
  ...
  if (debug_)
    LogDebug("DTPattern")<< "Pattern id: " << std::get<0>(id_) << " , " << std::get<1>(id_) << " , " << std::get<2>(id_);
}
...
void DTPattern::addHit(RefDTPatternHit hit) {
  ...
  if (debug_)
    LogDebug("DTPattern")<< "Added gen hit: " << std::get<0>(hit) << " , " << std::get<1>(hit) << " , " << std::get<2>(hit);
}
...
std::ostream &operator<<(std::ostream &out, DTPattern &p) {
  ...
  out << "Pattern id: " << std::get<0>(p.id()) << " , " << std::get<1>(p.id()) << " , " << std::get<2>(p.id()) << std::endl;
