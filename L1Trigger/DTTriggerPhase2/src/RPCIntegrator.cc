  ...
  for (const auto& rpcIt : *rpcRecHits) {
    ...
    RPCMetaprimitives_.emplace_back(
        RPCMetaprimitive(rpcDetId,
                         ...
                         global_position,
                         3, 
                         rpcIt.BunchX() + BX_SHIFT,
                         rpcIt.time() + BX_SHIFT * LHC_CLK_FREQ));   
  } 
}
...
void RPCIntegrator::makeRPCOnlySegments() {
  ...
  for (auto rpc_mp_it_layer1 = RPCMetaprimitives_.begin(); rpc_mp_it_layer1 != RPCMetaprimitives_.end();
    ...
    if (bestMatch_rpc_mp_layer2) {
      ...
      L1Phase2MuDTPhDigi rpc_only_segment = createL1Phase2MuDTPhDigi(rpc_id_l1, rpc_mp_it_layer1->rpc_bx, t0, global_phi, phiB, 2);  
...
RPCMetaprimitive* RPCIntegrator::matchDTwithRPC(metaPrimitive* dt_metaprimitive) {
  ...
  for (auto rpc_mp_it = RPCMetaprimitives_.begin(); rpc_mp_it != RPCMetaprimitives_.end(); rpc_mp_it++) {
    ...
    if (rpc_det_id.ring() == dt_chId.wheel()  // ring() in barrel RPC corresponds to the wheel
      ...
      int delta_phi = (int)round( (phi_DT_MP_conv(rpc_mp_it->global_position.phi(), rpc_det_id.sector()) - dt_metaprimitive->phi) * m_dt_phiB_granularity_);  
