...
void MuonPathAssociator::correlateMPaths(edm::Handle<DTDigiCollection> dtdigis,
  ...
  for (int wh = -2; wh <= 2; wh++) {   //wheel num: -2, -1, 0, +1, +2
	...
	//        for (auto metaprimitiveIt = inMPaths.begin(); metaprimitiveIt != inMPaths.end(); ++metaprimitiveIt
        ...
        if (debug_)
          LogDebug("MuonPathAssociator") << "correlating " << SL1metaPrimitives.size() << " metaPrim in SL1 and " << SL3metaPrimitives.size()
                    << " in SL3 for " << sl3Id ;
        ...
        for (auto SL1metaPrimitive = SL1metaPrimitives.begin(); SL1metaPrimitive != SL1metaPrimitives.end();
          ...
          for (auto SL3metaPrimitive = SL3metaPrimitives.begin(); SL3metaPrimitive != SL3metaPrimitives.end();
            ...
            if (useBX_correlation_) {
              if (abs(round(SL1metaPrimitive->t0 / (float)LHC_CLK_FREQ) - round(SL3metaPrimitive->t0 / (float)LHC_CLK_FREQ)) > dBX_correlate_TP_)
                continue;  //BX match
            } else {
        ...
        if (at_least_one_correlation == false || clean_chi2_correlation_) {
          if (debug_ && !at_least_one_correlation)
            LogDebug("MuonPathAssociator") << "correlation we found zero correlations, adding both collections as they are to the outMPaths";
          if (debug_)
            LogDebug("MuonPathAssociator") << "correlation sizes:" << SL1metaPrimitives.size() << " " << SL3metaPrimitives.size();
...
void MuonPathAssociator::removeSharingFits(vector<metaPrimitive> &chamberMPaths,
                                           vector<metaPrimitive> &allMPaths) {
