...
void MuonPathAnalyzerInChamber::analyze(MuonPathPtr &inMPath, MuonPathPtrs &outMPath) {
  ...
  if (debug_) {
    ...
    for (int i = 0; i < mPath->nprimitives(); i++)
      LogDebug("MuonPathAnalyzerInChamber")<< mPath->primitive(i)->layerId() << " , " << mPath->primitive(i)->superLayerId() << " , "
  ...
  if (mpAux != NULL) {
    ...
    if (debug_)
      LogDebug("MuonPathAnalyzerInChamber")<< "DTp2:analize \t\t\t\t\t Laterality " << bestI << " is the one with smaller chi2" ;
  } else {
    if (debug_)
      LogDebug("MuonPathAnalyzerInChamber")<< "DTp2:analize \t\t\t\t\t No Laterality found with chi2 smaller than threshold" ;
  }
  if (debug_)
...
void MuonPathAnalyzerInChamber::setWirePosAndTimeInMP(MuonPathPtr &mpath) {
  ...
  if (debug_)
    LogDebug("MuonPathAnalyzerInChamber")<< "Id " << chId.rawId() << " Wh " << chId.wheel() << " St " << chId.station() << " Se " << chId.sector();
  ...
  if (debug_)
    LogDebug("MuonPathAnalyzerInChamber")<< "shift1=" << shiftinfo_[wireId1.rawId()] << " shift3=" << shiftinfo_[wireId3.rawId()] ;
...
void MuonPathAnalyzerInChamber::calculateFitParameters(MuonPathPtr &mpath,
  ...
  for (int lay = 0; lay < 8; lay++) {
    if (debug_)
      LogDebug("MuonPathAnalyzerInChamber")<< "In fitPerLat " << lay << " xwire " << xwire[lay] << " zwire " << zwire[lay] << " tTDCvdrift "
           << tTDCvdrift[lay] ;
  ...
  for (int lay = 0; lay < 8; lay++) {
    if (present_layer[lay] == 0)
      continue;
    if (debug_)
      LogDebug("MuonPathAnalyzerInChamber")<< " For layer " << lay + 1 << " xwire[lay] " << xwire[lay] << " zwire " << zwire[lay] << " b " << b[lay];
    if (debug_)
  ...
  if (debug_) {
    ...
    LogDebug("MuonPathAnalyzerInChamber")<< "rect0 " << rect0vdrift / DRIFT_SPEED << " recBX " << rect0vdrift / DRIFT_SPEED / 25 << " recslope "
  ...
  if (debug_)
    LogDebug("MuonPathAnalyzerInChamber")<< "In fitPerLat Chi2 " << recchi2 << " with sign " << sign_tdriftvdrift << " within cell "
  ...
  if (!(sign_tdriftvdrift == -1) && !(incell_tdriftvdrift == -1) && !(physical_slope == -1)) {
    ...
    if (debug_)
      LogDebug("MuonPathAnalyzerInChamber") << "In fitPerLat "
