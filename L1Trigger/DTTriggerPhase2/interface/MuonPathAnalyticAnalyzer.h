#ifndef L1Trigger_DTTriggerPhase2_MuonPathAnalyticAnalyzer_cc
#define L1Trigger_DTTriggerPhase2_MuonPathAnalyticAnalyzer_cc
...
public:
  ...
  virtual ~MuonPathAnalyticAnalyzer();
...
private:
  ...
  int totalNumValLateralities_ = 16;
  ...
  //std::map<std::pair<int*,int*>, std::vector<latcomb_consts>> LAYOUT_VALID_TO_LATCOMB_CONSTS_ENCODER;
  //std::map<cell_valid, std::vector<latcomb_consts>> LAYOUT_VALID_TO_LATCOMB_CONSTS_ENCODER;
  ...
  void superlayer_datapath(MuonPath *mPath);
  void segment_composer(int valids[4], int wires[4], int t0s[4], int cell_horiz_layout[4], MuonPath *mPath);
  int validate_4hits_one_latcomb_reduced(int valids[4], int t0s[4], int cell_horiz_layout[4], latcomb_consts my_latcomb_consts);
  int tuck_t0(int t, int t0s[4], int valid[4]);
  int changeLatToEmu (int lat);
/*
bool operator <(const cell_valid &o, const cell_valid &p) {
  for (int i = 0; i<4; i++) {
    if ( !( (o.cell_horiz_layout[i] < p.cell_horiz_layout[i]) || ( (o.cell_horiz_layout[i] == p.cell_horiz_layout[i]) &&  o.valid[i] < p.valid[i]) ) ) return false; 
  }
  return true;
} */
};
